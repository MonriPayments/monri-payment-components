    ...
    withMethods(
        (
            ...
            const onApplePayButtonClick = () => {
                applePayService.startPayment(store.inputParams()).pipe(take(1)).subscribe(response => {
                    console.log("Response:", response);
                    const request = {
                        countryCode: store.appleButtonConfig()?.countryCode || 'HR',
                        currencyCode: store.appleButtonConfig()?.currencyCode || 'EUR',
                        supportedNetworks: store.appleButtonConfig()?.supportedNetworks || ['visa', 'masterCard', 'amex', 'discover'],
                        merchantCapabilities: store.appleButtonConfig()?.merchantCapabilities || ['supports3DS'],
                        total: {
